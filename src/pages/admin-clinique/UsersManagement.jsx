...
export default function UsersManagement() {
  ...
  // Recharger les utilisateurs quand les spécialités changent pour l'enrichissement
  useEffect(() => {
    if (specialities.length > 0 && users.length > 0) {
      // Force re-render when both are loaded
      setUsers(prevUsers => [...prevUsers]);
    }
  }, [specialities]);

  // Fonction pour attendre que les spécialités soient chargées
  const waitForSpecialities = async (maxAttempts = 10, delay = 100) => {
    for (let i = 0; i < maxAttempts; i++) {
      if (specialities.length > 0) return specialities;
      await new Promise(resolve => setTimeout(resolve, delay));
    }
    return specialities; // Retourne même si vide après le délai
  };
  ...
  const loadUsers = async () => {
    try {
      setLoading(true);
      const data = await userService.getAllUsersWithDetails();
      console.log('📊 Raw users data from API:', data);
      console.log('📊 First user example:', data[0]);
      console.log('📊 Looking for doctors:', data.filter(u => u.role === 'MEDECIN' || u.role === 'ROLE_MEDECIN'));
  ...
  const handleOpenModal = async (userToEdit = null) => {
    if (userToEdit) {
      console.log('🎯 handleOpenModal called with user:', userToEdit);
      console.log('🎯 User object keys:', Object.keys(userToEdit));
      console.log('🎯 User role specifically:', userToEdit.role);
      console.log('🎯 User specialityId specifically:', userToEdit.specialityId);
      setEditingUser(userToEdit);

      // Attendre que les spécialités soient chargées si nécessaire
      const currentSpecialities = await waitForSpecialities();
      console.log('🔍 Specialities available:', currentSpecialities);

      // Pour l'édition, trouver le nom de la spécialité à partir de l'ID
      let specialityName = '';
      if (userToEdit.specialityId && currentSpecialities.length > 0) {
        const speciality = currentSpecialities.find(s => s.id === userToEdit.specialityId);
        specialityName = speciality?.name || '';
        console.log('🔍 Found speciality:', speciality);
      }

      // Normaliser le rôle - garder la valeur originale
      const normalizedRole = userToEdit.role || 'MEDECIN';
      console.log('🔍 Original role:', userToEdit.role);
    ...
    setShowModal(true);
    setModalKey(prev => prev + 1); // Force re-render du modal
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingUser) {
        ...
        // Le backend attend le rôle sans le préfixe ROLE_
        // Pas de conversion nécessaire, le rôle est déjà au bon format
  ...
  // Enrichir les utilisateurs avec les noms des spécialités
  const enrichedUsers = useMemo(() => {
    return users.map(user => {
      if (user.role === 'MEDECIN' && user.specialityId && specialities.length > 0) {
        const speciality = specialities.find(s => s.id === user.specialityId);
        return {
          ...user,
          specialityName: speciality?.name || 'Spécialité inconnue'
        };
      }
      return user;
    });
  ...
  }, [enrichedUsers, searchTerm, selectedRole, selectedStatus]);
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          {user.role === 'MEDECIN' && user.specialityName && (
