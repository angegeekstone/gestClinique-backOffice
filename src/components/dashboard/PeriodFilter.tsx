...
import { usePeriodFilter, type PeriodPreset } from '../../hooks/usePeriodFilter';
...
export default function PeriodFilter(): JSX.Element {
  ...
  const [tempStartDate, setTempStartDate] = useState<string>(dateRange.startDate || '');
  ...
  const handleCustomDatesApply = (): void => {
  ...
  const handleCustomDatesCancel = (): void => {
    setShowCustomDates(false);
    setTempStartDate(dateRange.startDate || '');
    setTempEndDate(dateRange.endDate || '');
  };
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 bg-white rounded-md shadow-lg border border-gray-200 z-50">
          <div className="py-1">
            ...
            {Object.entries(presetLabels).map(([preset, label]: [string, string]) => {
              ...
              return (
                <button
                  key={preset}
                  onClick={() => handlePresetClick(preset as PeriodPreset)}
